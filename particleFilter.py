...
class robot:
    ...
    def move(self, motion): 
        ...
        temp_robot = robot()
        temp_robot.length = self.length
        temp_robot.bearing_noise = self.bearing_noise
        temp_robot.steering_noise = self.steering_noise
        temp_robot.distance_noise = self.distance_noise
        ...
        if turn < 0.001:  # Approximately linear motion
            ...
            temp_robot.y = cy - cos(self.orientation + turn) * radius
            temp_robot.orientation = (self.orientation + turn) % (2 * pi)
